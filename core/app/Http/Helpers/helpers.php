...
function real_time_invest($milliseconds, $id)
{
    ...
    // Event::setTimer()
       while(true)
       {
           ...
           {
               if($invest->next_time == date("Y-m-d h:i:sa"))
               {
                   // Add 24hrs to next time
                   $now = Carbon::now();
                   ...
                   while (0 == 0) {
                    ...
                    $transaction->details = $invest->interest . "Interest from FORTOREMALL";
                    $transaction->save();
                    // End Save //
...
function reCaptcha()
{
    $reCaptcha = Plugin::where('act', 'google-recaptcha2')->where('status', 1)->first();
    return $reCaptcha ? $reCaptcha->generateScript() : '';
}

function analytics()
{
    $reCaptcha = Plugin::where('act', 'google-analytics')->where('status', 1)->first();
    return $reCaptcha ? $reCaptcha->generateScript() : '';
}

function fbComment()
{
    $fbComment = Plugin::where('act', 'fb-comment')->where('status', 1)->first();
    return $fbComment ? $fbComment->generateScript() : '';
}

function tawkto()
{
    $reCaptcha = Plugin::where('act', 'tawk-chat')->where('status', 1)->first();
    return $reCaptcha ? $reCaptcha->generateScript() : '';
}
...
{
    $captcha = Plugin::where('act', 'custom-captcha')->where('status', 1)->first();
    $gnl = GeneralSetting::first();
...
function getImage($image): string
{
    ...
    return 'https://ui-avatars.com/api/?name=' . urlencode($image) . '&color=7F9CF5&background=EBF4FF';
}

function getImageBase64($image): string
{
    return 'data:image/image/jpeg;base64,' . base64_encode(file_get_contents(base_path("../$image")));
}
...
function send_sms_message($mobile,$message)
{
    $sendSms = new SendSms;
    $sendSms->ourSms($mobile, $message);
}
function send_contract_otp(User $user, Invest $invest, $otp)
{
    $sendSms = new SendSms;
    $sendSms->ourSms($user->mobile, "Hello {$user->fullname()}, Your Contract {$invest->trx} Verification Code Is {$otp}");
}
...
function send_sms($user, $type, $shortCodes = [])
{
    ...
    if ($smsTemplate) {
        ...
        $sendSms->ourSms($user->mobile, $message);
    }
}

function send_email($user, $type = null, $shortCodes = [])
{
    $general = GeneralSetting::getActiveSetting();
    ...
    SendEmailJob::dispatch($config, $user->email, $user->username, $general->email_from, $general->sitetitle, $email_template->subj, $message, $general);
}
...
function send_general_email($email, $subject, $message, $receiver_name = '')
{
    ...
    SendEmailJob::dispatch($config, $email, $receiver_name, $general->email_from, $general->sitetitle, $subject, $message, $general);
}
...
function showBelowUser($id)
{
    ...
    foreach ($under_ref as $data) {
        $cc = User::where('ref_by', $data->id)->count();
