            ...
            <div class="row justify-content-center mt-2">
                <div class="col-md-12">
                    <div class="right float-right mb-5">
                        <a href="{{ route('user.plan') }}" class="btn cmn-btn">
                            @lang('Invesment Plan')
                        </a>
                    </div>
                </div>
                    ...
                    <div class="table-responsive--md">
                        <table class="table style--two">
                            ...
                            @forelse($logs as $k=>$data)
                                <tr>
                                    <td data-label="@lang('Plan')">{{trans(optional($data->plan)->name)}}
                                        <br> {{getAmount($data->amount)}}  {{__($general->cur_text)}} </td>
                                    <td data-label="@lang('Return')">
                                        {{getAmount($data->interest)}} {{__($general->cur_text)}}
                                        every {{$data->time_name}}
                                        <br>
                                        for
                                        @if($data->period == '-1')
                                            @lang('Lifetime')
                                    ...
                                    <td data-label="@lang('Received')">  {{$data->return_rec_time}}
                                        x{{ $data->interest }}
                                        = {{$data->return_rec_time*$data->interest }} {{__($general->cur_text)}} </td>
                                    <td data-label="@lang('Contract')">
                                        @if($data->hasSignedByUser() && $data->contract_path)
                                            <a href="{{$data->contractUrl()}}">Contract</a>
                                        @endif
                                    </td>
